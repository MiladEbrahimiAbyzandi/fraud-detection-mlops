...
def transform_stage1(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["timestamp"] = pd.to_datetime(
        df["datetime_str"], format="%Y-%m-%d %H:%M"
    )
    ...
    # Transaction Type
    df["transaction_type"] = df["Merchant_City"].apply(
        lambda x: "CNP" if x.strip().lower() == "online" else "CP"
    )
    ...
    df["high_debt_ratio"] = np.where(
        df["debt_to_income"] > 2 & (df["debt_to_income"].notna()),
        1,
        0
    )
    df["high_debt_high_spend"] = (
        (df["Total_Debt"] > 0.5 * df["Yearly_Income_Person"])
        & (df["Amount"] > 0.1 * df["Yearly_Income_Person"])
    ).astype(int)
    df["credit_utilization"] = (
        df["Total_Debt"] / df["Credit_Limit"]
    )
    df["credit_utilization"] = df["credit_utilization"].replace(
    ...
    df["low_FICO_high_DTI"] = (
        (df["FICO_Score"] < 670) & (df["debt_to_income"] > 2)
    ).astype(int)

    # Synthetic fraud
    df["account_tenure_years"] = (
        -(df["Acct_Open_Date"] - df["timestamp"]).dt.days / 365
    )
    df["synthetic_risk"] = (
        (df["FICO_Score"] >= 740) & (df["account_tenure_years"] < 2)
    ).astype(int)
    df["synthetic_risk_2"] = (
        (df["Num_Credit_Cards"] > 4) & (df["high_debt_ratio"] == 1)
    ).astype(int)
    # Zip mismatch
    df["is_us_zip"] = df["Zip"].str.match(r"^\d{4,5}$")
    df["zip_mismatch_flag"] = (
        (df["Zip"] != df["Zipcode"]) & df["is_us_zip"]
    ).astype(int)

    # Expiry
    df["months_to_expiry"] = (
        df["Expires"] - df["timestamp"]
    ).dt.days / 30
