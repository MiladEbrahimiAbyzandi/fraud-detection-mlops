...
def transform_stage2( metadata: dict, data:pd.DataFrame | pd.Series ) -> pd.DataFrame |pd.Series:
    try:
        ...
        data["high_risk_state"] = data["Merchant_State"].apply(
            lambda x: 1 if x in high_risk_states else 0
        )
        data["high_risk_cities"] = data["Merchant_City"].apply(
            lambda x: 1 if x in high_risk_cities else 0
        )
        data["high_risk_transactions"] = (
            data["amount_income_ratio"] > threshold
        ).astype(int)
        data["high_risk_MCC"] = data["MCC"].apply(
            lambda x: 1 if x in high_risk_mcc else 0
        )
        ...
        data["time_diff"] = (
            data.groupby("User")["timestamp"].diff(-1).abs()
        )
        data["rapid_mcc_changed"] = (
            (data["mcc_changed"] == 1)
            & (data["time_diff"] <= pd.Timedelta(hours=1))
        ).astype(int)

        data["high_risk_merchant"] = data["Merchant_Name"].apply(
            lambda x: 1 if x in high_risk_merchants else 0
        )
