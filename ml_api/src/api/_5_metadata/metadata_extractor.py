...
def metadata(X_train: pd.DataFrame, y_train: pd.Series) -> dict:
    ...
    try:
        ...
        state_fraud_rate = cp_transactions.groupby("Merchant_State")["Is_Fraud"].mean()
        high_risk_states = state_fraud_rate[state_fraud_rate > df["Is_Fraud"].mean()].index.tolist()

        # High-risk cities
        city_fraud_rate = cp_transactions.groupby("Merchant_City")["Is_Fraud"].mean()
        high_risk_cities = city_fraud_rate[city_fraud_rate > df["Is_Fraud"].mean()].index.tolist()

        df["amount_income_ratio"] = df["Amount"] / df["Yearly_Income_Person"]
        threshold = df["amount_income_ratio"].quantile(0.99)

        # MCC features
        mcc_fraud_data = df.groupby("MCC").agg(fraud_count=("Is_Fraud", "sum"), fraud_rate=("Is_Fraud", "mean"))
        high_risk_mcc = mcc_fraud_data[mcc_fraud_data["fraud_rate"] > df["Is_Fraud"].mean()].index.tolist()

        # High-risk merchants
        fraud_stats_by_merchant = (
            df.groupby("Merchant_Name")["Is_Fraud"].agg(["count", "mean"]).sort_values(by="mean", ascending=False)
        )
        fraud_stats_by_merchant.columns = ["transaction_count", "fraud_rate"]
        high_risk_merchants = fraud_stats_by_merchant[
            (fraud_stats_by_merchant["transaction_count"] >= 20) & (fraud_stats_by_merchant["fraud_rate"] > 0.1)
        ].index
        ...
        metadata_dict = {
            ...
            "high_risk_merchants": list(high_risk_merchants),  # convert Index to list
            "threshold": threshold,
        }
